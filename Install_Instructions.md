...
## Second sitting - Provision your Amazon EC2 instance
...
Copy/paste or type out the following commands (hit Enter and wait for each one to finish before running the next one)
```
...
More text will scroll – occasionally it will stop scrolling for a minute – be patient. If everything is going well you will get another bright pink or blue screen. **Type in your domain name and press enter.** (What happens if I mis-type?)

Almost immediately, another pink or blue screen will say **“Generate a new self-signed certificate ….”** Press enter. We will change this to a real certificate later.

More text…. Be patient. Next you will get a prompt:
```
Enter your email and press [ENTER]:
```
Enter the email address associated with your domain name and press enter. (What if I mis-type?)

Then lots more text. You may see a scary ```[WARNING]: provided hosts list is empty, only localhost is available. Note that the implicit localhost does not match 'all'``` but that is normal. The Init AIDE task will also take several minutes, so be patient if it appears to hang.
